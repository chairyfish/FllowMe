package com.example.melvin.fllowme.routing;
...
public class LinkRouteActivity extends Activity implements
        RoutingListFragment.onDrawerListSelectedListener,
        com.example.melvin.fllowme.routing.RoutingDialogFragment.onGetRoute {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_link_route);
...
/////////////////////////////////////////////
        ...
        stopSymbol = new SimpleMarkerSymbol(Color.RED, 15, SimpleMarkerSymbol.STYLE.CIRCLE);
/////////////////////////////////////////////////
        ...
        img_getDirections.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                dialog = ProgressDialog.show(LinkRouteActivity.this, "查找路径",
                        "正在计算，请稍候...", true);
                if (stopPoints.size() < 2) {
                    Toast.makeText(getApplicationContext(), "必须至少选择两个停靠点", Toast.LENGTH_SHORT).show();
                    return;
                }
//                Toast.makeText(getApplicationContext(), "开始查找最短路径", Toast.LENGTH_LONG).show();
        ...
        map.setOnSingleTapListener(new OnSingleTapListener() {
            ...
            public void onSingleTap(float x, float y) {
                if (hiddenSegmentsLayer.getNumberOfGraphics() == 0) {//增加停靠点
                    Point mapPoint = map.toMapPoint(x, y);
                    routeLayer.addGraphic(new Graphic(mapPoint, stopSymbol));
                    stopPoints.add((Point) GeometryEngine.project(mapPoint, wm, egs));
                    Point p = (Point) GeometryEngine.project(mapPoint, wm, egs);
                    Log.i("point", String.valueOf(p.getX()) + "," + String.valueOf(p.getY()));
                } else {
                    ...
                    String label = String.format("长度：" + length + "米,  时间：" + time + "分钟,描述：" + direction);
    ...
    private void QueryDirections(final Point mLocation, final Point p) {
        ...
        dialog = ProgressDialog.show(LinkRouteActivity.this, "Routing Sample",
    ...
    void updateUI() {
        ...
        if (mResults == null) {
            Toast.makeText(LinkRouteActivity.this, mException.toString(),
